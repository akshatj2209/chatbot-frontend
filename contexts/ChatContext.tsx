...
export const ChatContextProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const convertToLinearStructure = (messages: ConversationMessage[]): Message[] => {
        ...
        while (currentMessage) {
            const currentVersion = currentMessage.versions.find((v: Version) => v.id === currentMessage?.current_version);
            result.push({
            ...
            currentMessage = childIds.length > 0 ? messageMap.get(childIds[0]) as ConversationMessage : undefined;
        }
    ...
    const updateMessage = async (messageId: string, newContent: string) => {
        try {
            const response = await fetch('/api/chat', {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ messageId: messageId, message: newContent, conversationId: conversationId }),
            });
